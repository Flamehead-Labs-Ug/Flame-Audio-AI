...
from flameaudio import load_agents, AUTH_ENABLED
...
# Configure styling for chat messages
st.markdown("""
<style>
.chat-message {
...
# Custom function to display chat messages
def display_message(role, content, metadata=None):
    if role == "user":
        avatar_img = "logos/user_avatar.png"  # Default user avatar
        background = "user"
        prefix = "You"
    elif role == "assistant":
        avatar_img = "logos/flame logo.jpg"  # Agent avatar
        background = "assistant"
        prefix = "Flame Agent"
    elif role == "system":
        avatar_img = "logos/system_avatar.png"  # System avatar
        background = "system"
        prefix = "System"
    else:
        avatar_img = "logos/flame logo.jpg"
        background = "assistant"
    ...
    <div class="chat-message {background}">
        <div class="avatar">
            <img src="{avatar_img}" style="max-height: 78px; max-width: 78px; border-radius: 50%; object-fit: cover;">
        </div>
        <div class="message">{content}</div>
    </div>
    """, unsafe_allow_html=True)
    
    # Add document sources expander for assistant messages if we have source documents in metadata
...
if st.session_state.get("chat_agent"):
    ...
    # Function to load chat sessions for the current agent
    def load_chat_sessions():
        try:
            response = requests.get(
                f"{BACKEND_URL}/chat/sessions?agent_id={st.session_state.get('chat_agent', '')}",
                headers={
            ...
            if config_tab == "Agent config":
                ...
                if not AUTH_ENABLED or st.session_state.get("authenticated", False):
                    ...
                    if st.session_state.chat_agent:
                        ...
                        doc_options = [{"label": "All Documents (Agent Knowledge)", "value": ""}]
                        ...
                        selected_doc = st.selectbox(
                            ...
                            index=next((i for i, d in enumerate(doc_options) if d["value"] == st.session_state.chat_document), 0),
                        ...
                        if not selected_doc:
                            st.info("The agent will have access to all documents associated with it.")
                        else:
...
if not AUTH_ENABLED or st.session_state.get("authenticated", False):
    ...
    def send_message(message, session_id=None):
        ...
        if st.session_state.chat_document:
            chat_data["document_id"] = st.session_state.chat_document
            
        # Add metadata
        chat_data["metadata"] = {}
            
        # Add system prompt if overridden
        system_prompt = st.session_state.get("chat_system_prompt", "")
        if system_prompt:
            # Get original system prompt from agent
            original_prompt = ""
        ...
        # Add current user message to chat history for immediate display
        ...
        try:
            with st.spinner("Agent is thinking..."):
                ...
                if response.status_code == 200:
                    ...
                    # Fetch complete chat history from backend to ensure consistency
    ...
    if not st.session_state.chat_agent:
        ...
        with chat_col:
            ...
            st.subheader(f"Chat with {agent_name}{doc_label}")
            ...
            if user_input:
                ...
                send_message(user_input, st.session_state.chat_session_id)
